 ...
 * `INVENTORY_ROOTS` and nothing in `use-invalidate-inventory.ts` touches these.
 * Moving a box must not refetch a list of credentials, and issuing a credential
 * must not invalidate the forest.
 *
 * ## Why none of it is cached beyond the query client
 *
 * The list is account state. It is never in the service worker's runtime cache
 * — `pwa-caching.ts` matches `/photos` and `/(storage-units|items)` and nothing
 * else, which `pwa-caching.test.ts` pins — so a stale list of credentials
 * cannot be served to somebody deciding which one to revoke.
 *
 * `staleTime: 0` on top of that: the app's default is 30 seconds, which is
 * right for a shelf of boxes and wrong for the screen somebody opens BECAUSE
 * they think a credential has leaked.
 */
export const useMachineTokens = (): UseQueryResult<MachineTokenListResponse> => {
  const api = useApi();
 ...
 *
 * Neither of them writes the answer into the query cache, and that is
 * deliberate rather than an omission: the query cache is a store with a
 * lifetime nobody is thinking about while they read a warning that says "copy
 * this now". The secret goes into component state, is rendered, and goes when
 * the component does. The LIST is invalidated instead, which is the part that
 * is safe to keep.
 */
