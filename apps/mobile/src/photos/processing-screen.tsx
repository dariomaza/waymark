 ...
 * `FAILED` photo is not failed for ever (ADR 4). On a phone none of the three
 * had a button anywhere — which is the odd way round, because the phone is
 * the machine in the hand of somebody who has just watched a background
 ...
 * that must never take space from the primary one, and a sixth destination
...
export const PhotoProcessingScreen = (): JSX.Element => {
  ...
  return (
    <Screen>
      <ScreenTitle>{t("photos.processingTitle")}</ScreenTitle>
        ...
        <ProcessingDetail
          ...
          rowAction={(photoId) => (
            <Button
              ...
              label={t("photos.retryNamed", { photoId })}
              onPress={() => {
      ...
      */}
      {retryAll.isSuccess ? (
        <Callout tone="note">{t("photos.requeued", { count: retryAll.data.requeued })}</Callout>
      ) : null}
