 ...
 *
 * The alt text is empty on purpose. The name is right underneath inside the
 * same card, so describing the picture as well would announce the same thing
...
export const ItemCover = ({ item }: ItemCoverProps): JSX.Element | null => {
  ...
  return cover === undefined ? null : <AuthenticatedImage src={cover.thumbnailUrl} alt="" fill />;
};
